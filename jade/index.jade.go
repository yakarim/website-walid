...
const (
	...
	index__2 = `</ul><form class="d-flex"><input class="form-control me-2" type="search" placeholder="Search" aria-label="Search"/><button class="btn btn-outline-success" type="submit">Search</button></form></div></div></nav><div class="container"><h1>My Article</h1><div class="footer">2021</div></div></body></html>`
	index__3 = `<li class="nav-item"></li><a class="nav-link" href="./admin">Admin</a><a class="nav-item nav-link" href="./login">Logout (`
	index__4 = `)</a>`
	index__5 = `<a class="nav-item nav-link" href="./login">Login</a>`
)

func Index(pageTitle, user string, signIn bool, wr io.Writer) {
